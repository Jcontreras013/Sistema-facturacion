...
- **Órdenes de compra a proveedores (solo administradores):** crea una orden en borrador, agrega los productos con cantidad y costo, márcala como enviada, y registra la recepción de mercancía (total o parcial). Cada recepción genera automáticamente el movimiento de inventario correspondiente y suma al stock — no hay que capturar el ingreso de mercancía dos veces. Si el producto que llegó todavía no existe en el catálogo (proveedor nuevo, artículo nuevo), hay un botón "¿No está en la lista? Crear producto nuevo" que abre el formulario de producto con el proveedor de la orden ya seleccionado, y al guardar regresa directo a la orden con el producto listo para agregar.
- **Conteo físico de inventario (solo administradores):** al iniciar un conteo, el sistema toma una foto del stock actual de todos los productos activos. Puedes ir ingresando la cantidad real que encuentras en bodega y guardar tu progreso las veces que quieras; la diferencia contra el sistema se muestra al instante. Al cerrar el conteo, el inventario se ajusta automáticamente a lo contado (con el movimiento de inventario correspondiente) y el conteo queda bloqueado para consulta.
