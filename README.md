...
- **Inventario:** productos con código/SKU, código de barras, categoría, proveedor, precios de compra/venta, ISV, stock, fecha de vencimiento (basta con llenar el código de barras o el SKU, no ambos — si dejas el SKU en blanco y escaneas o escribes el código de barras, el sistema genera el SKU solo); alertas de stock bajo y de productos por vencer; movimientos de inventario con motivo (compra, venta, merma, daño, robo, devolución, otro). Cada producto puede tener un precio de mayoreo opcional (precio + cantidad mínima); en el POS se aplica automáticamente en cuanto la cantidad de esa línea alcanza el mínimo, con una etiqueta "Mayoreo" para que quede claro por qué cambió el precio.
