...
- **Inventario:** productos con código/SKU, código de barras, categoría, proveedor, precios de compra/venta, ISV, stock, fecha de vencimiento (basta con llenar el código de barras o el SKU, no ambos — si dejas el SKU en blanco y escaneas o escribes el código de barras, el sistema genera el SKU solo). El código de barras ya no tiene que ser único: si escribes uno que ya está registrado en otro producto (por ejemplo una variante de empaque del mismo proveedor que reutiliza el código), aparece un aviso indicando en qué producto ya existe, pero puedes guardar igual; alertas de stock bajo y de productos por vencer; movimientos de inventario con motivo (compra, venta, merma, daño, robo, devolución, otro). Cada producto puede tener un precio de mayoreo opcional (precio + cantidad mínima); en el POS se aplica automáticamente en cuanto la cantidad de esa línea alcanza el mínimo, con una etiqueta "Mayoreo" para que quede claro por qué cambió el precio.
- **Promociones (solo administradores):** descuentos porcentuales programados por producto específico o por categoría completa, con fecha de inicio y fin. Mientras la promoción esté vigente, el POS aplica el precio rebajado automáticamente al agregar el producto (con una etiqueta "Promoción"), sin que el cajero tenga que hacer nada. Si un producto califica para mayoreo y también tiene una promoción activa al mismo tiempo, gana el precio de mayoreo.
...
- **Modo sin conexión (PWA) en el punto de venta:** el sistema es instalable como app (ícono en el celular/tablet/escritorio) y funciona como PWA. Si se pierde el internet en plena venta, el POS sigue permitiendo cobrar: la venta se guarda en el dispositivo (IndexedDB) y se envía sola al servidor en cuanto vuelve la conexión, sin perder la venta ni bloquear al cajero. Un indicador en pantalla muestra si hay conexión y cuántas ventas están pendientes de sincronizar. **Limitaciones:** el catálogo de productos/stock que se ve sin conexión es el de la última vez que esa pantalla cargó con internet, así que el stock mostrado puede estar desactualizado; y está pensado para un solo dispositivo cobrando a la vez (si varios cajeros venden el mismo producto sin conexión simultáneamente, el control de stock se reconcilia hasta que todos sincronizan).
- **Facturación en contingencia sin internet (solo administradores, opcional):** en Admin → Configuración del negocio puedes activar un **rango de contingencia** — un bloque de correlativos aparte del rango normal, reservado exclusivamente para cuando se cae el internet. Con esto activado, si el POS pierde la conexión, cada venta recibe de inmediato un número de factura real de ese bloque y se **imprime en el momento** un comprobante marcado como "Emitida en contingencia", en vez de dejar al cliente sin ningún papel. Al volver el internet la venta se sincroniza con ese mismo número, queda marcada como "Contingencia" en el historial de ventas, y el servidor valida que el número esté dentro del rango y no se haya usado antes (rechaza duplicados). Si no activas esta opción, una venta sin conexión sigue funcionando igual que antes: se guarda y se sincroniza sola, pero sin número de factura ni impresión hasta que recupera la conexión. **Importante:** esto asume una sola caja/dispositivo facturando en contingencia a la vez — si dos cajas quedan sin internet al mismo tiempo y ambas facturan en contingencia, podrían coincidir en el mismo número; para negocios con más de una caja, o si no estás seguro de qué permite tu resolución de CAI, conviene confirmar con tu contador si te conviene más este rango digital o el respaldo tradicional de talonario físico.
